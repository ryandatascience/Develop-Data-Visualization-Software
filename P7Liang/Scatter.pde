...
class Scatter {
  ...
  void draw() {
    if ( data == null ) return;
      ...
      text("Please select a data point from the scatter plot (upper right)", width/10, height-2*Xbuffer);
      text("Selected Row #  "+ selected, width/10, height-Xbuffer);
      text("Pearson Correlation Coefficient Corrgram", 50, height-150);
    }
  }
}